...
function App() {
  ...
  useEffect(() => {
    ...
    setSlideIndex((current) => current % heroSlides.length);
  }, [heroSlides.length]);
  ...
  useEffect(() => {
    const currentSection = sections.find((section) => section.id === productDraft.sectionId);
    if (currentSection && !currentSection.subcategories.includes(productDraft.subcategory)) {
      setProductDraft((draft) => ({
        ...draft,
        subcategory: currentSection.subcategories[0] ?? draft.subcategory,
      }));
    }
  }, [productDraft.sectionId, productDraft.subcategory, sections]);

  useEffect(() => {
    if (sections.length && !sections.some((section) => section.id === selectedSection)) {
      setSelectedSection(sections[0].id);
    }
  }, [sections, selectedSection]);

  useEffect(() => {
    if (governorates.length && !governorates.includes(selectedProvince as (typeof governorates)[number])) {
      setSelectedProvince(governorates[0]);
    }
  }, [selectedProvince]);
